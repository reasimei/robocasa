#!/usr/bin/env python3
"""Compare DeliverStraw full-task control against oracle subtask switching.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--drawer-open-threshold", type=float, default=0.95)
    parser.add_argument(
        "--dining-counter-distance-threshold",
        type=float,
        default=0.20,
        help="Maximum XY base-position error for the navigation oracle.",
    )
    parser.add_argument(
        "--dining-counter-orientation-cos-threshold",
        type=float,
        default=0.98,
        help="Minimum yaw cosine similarity for the navigation oracle.",
    )
    parser.add_argument(
...
def unwrap_deliver_straw_env(adapter: RobocasaVectorEnvAdapter) -> Any:
    """Find the base DeliverStraw environment beneath vector and gym wrappers."""
    ...
    while current is not None and id(current) not in seen:
        ...
        if all(
            ...
            for attr in ("sim", "drawer", "dining_counter", "objects", "obj_body_id")
        ):
            return current
        current = getattr(current, "env", None)
    raise RuntimeError("Could not locate the base DeliverStraw environment through wrappers.")


def delivery_navigation_target(env: Any) -> tuple[np.ndarray, np.ndarray]:
    """Match the RoboCasa navigation target pose for the dining-counter glass."""
    target_pos, target_ori = EnvUtils.compute_robot_base_placement_pose(
        env,
        ref_fixture=env.dining_counter,
        ref_object="glass_cup",
    )
    return np.asarray(target_pos, dtype=float), np.asarray(target_ori, dtype=float)


def robot_near_dining_counter(
    env: Any,
    target_pos: np.ndarray,
    target_ori: np.ndarray,
    args: argparse.Namespace,
) -> tuple[bool, float, float]:
    """Use NavigateKitchen's mobile-base position and yaw checks."""
    robot_id = env.sim.model.body_name2id("mobilebase0_base")
    base_pos = np.asarray(env.sim.data.body_xpos[robot_id], dtype=float)
    position_error = float(np.linalg.norm(target_pos[:2] - base_pos[:2]))
    base_ori = T.mat2euler(
        np.asarray(env.sim.data.body_xmat[robot_id]).reshape((3, 3))
    )
    orientation_cos = float(np.cos(target_ori[2] - base_ori[2]))
    near = bool(
        position_error <= args.dining_counter_distance_threshold
        and orientation_cos >= args.dining_counter_orientation_cos_threshold
    )
    return near, position_error, orientation_cos


def deliver_straw_oracle_labels(
    env: Any,
    target_pos: np.ndarray,
    target_ori: np.ndarray,
    args: argparse.Namespace,
) -> dict[str, Any]:
    """Return DeliverStraw stage labels directly from simulator state."""
    drawer_state = env.drawer.get_door_state(env=env)
    drawer_open_fraction = float(min(drawer_state.values()))
    drawer_open = bool(drawer_open_fraction >= args.drawer_open_threshold)
    straw_grasped = bool(OU.check_obj_grasped(env, "straw"))
    near_dining_counter, position_error, orientation_cos = robot_near_dining_counter(
        env,
        target_pos,
        target_ori,
        args,
    )
    straw_in_glass_cup = bool(
        OU.check_obj_in_receptacle(env, "straw", "glass_cup", th=0.5)
    )
    gripper_far = bool(OU.gripper_obj_far(env, obj_name="straw"))
    env_check_success = bool(env._check_success())
    return {
        "open_drawer": drawer_open,
        "pick_straw_from_drawer": straw_grasped,
        "move_to_dining_counter": bool(straw_grasped and near_dining_counter),
        "place_straw_in_glass_cup": env_check_success,
        "drawer_open_fraction": drawer_open_fraction,
        "straw_grasped": straw_grasped,
        "near_dining_counter": near_dining_counter,
        "dining_counter_position_error": position_error,
        "dining_counter_orientation_cos": orientation_cos,
        "straw_in_glass_cup": straw_in_glass_cup,
...
def run_episode(
    ...
    base_env = unwrap_deliver_straw_env(env)
    navigation_target_pos, navigation_target_ori = delivery_navigation_target(base_env)
    subtask_index = 0
    ...
    initial_labels = deliver_straw_oracle_labels(
        base_env,
        navigation_target_pos,
        navigation_target_ori,
        args,
    )
    for subtask_id in (
        "open_drawer",
    ...
    while not done and simulator_steps < args.max_episode_steps:
        ...
        labels = deliver_straw_oracle_labels(
            base_env,
            navigation_target_pos,
            navigation_target_ori,
            args,
    ...
    final_labels = deliver_straw_oracle_labels(
        base_env,
        navigation_target_pos,
        navigation_target_ori,
        args,
...
def main() -> None:
    ...
    if actual_ids != expected_ids:
        raise ValueError(
            f"This oracle expects DeliverStraw subtasks {expected_ids}, got {actual_ids}. "
    ...
    write_json(
        mode_dir / "config.json",
        {
            **vars(args),
            "oracle_definition": {
                "open_drawer": (
                    "min(env.drawer.get_door_state(env).values()) "
                    ">= drawer_open_threshold"
                ),
                "pick_straw_from_drawer": "OU.check_obj_grasped(env, 'straw')",
                "move_to_dining_counter": (
                    "straw grasped, with mobilebase0_base within "
                    "dining_counter_distance_threshold of "
                    "EnvUtils.compute_robot_base_placement_pose("
                    "env, env.dining_counter, ref_object='glass_cup'), and "
                    "yaw cosine >= dining_counter_orientation_cos_threshold"
                ),
                "place_straw_in_glass_cup": (
                    "exactly DeliverStraw._check_success(): straw is in "
                    "glass_cup at th=0.5 and OU.gripper_obj_far(env, 'straw')"
                ),
            },
        },
    )
    ...
    for episode_index in range(args.n_episodes):
        ...
        if existing is not None and not args.overwrite:
            print(
                f"[deliverstraw-oracle] {args.mode} episode {episode_index + 1}/{args.n_episodes} "
        ...
        try:
            ...
            print(
                f"[deliverstraw-oracle] ERROR episode {episode_index}: {result['error']}",
                flush=True,
            )
        finally:
            if env is not None:
        ...
        if args.video:
            try:
                ...
                print(
                    f"[deliverstraw-oracle] VIDEO ERROR episode {episode_index}: "
        ...
        print(
            f"[deliverstraw-oracle] {args.mode} episode {episode_index + 1}: "
    ...
    write_json(mode_dir / "summary.json", summary)
    print(
        f"[deliverstraw-oracle] complete mode={args.mode} n={len(results)} "
